//! Playback State
//!
//! Playback state management for the timeline.
...
pub enum PlaybackState {
    /// Timeline is paused at the current playhead position
    #[default]
    Paused,
    /// Playing through historical events at the selected speed
    Playing,
    /// Following live time (real-time streaming mode)
    Following,
}

impl PlaybackState {
    /// Returns true if the timeline is actively advancing (Playing or Following)
    pub fn is_playing(&self) -> bool {
        matches!(self, Self::Playing | Self::Following)
    }

    /// Returns true if in live-following mode
    ...
    /// Get an icon for the current state
