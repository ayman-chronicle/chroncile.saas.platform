...
export async function GET(
  ...
  if (!token) {
    return NextResponse.redirect(
      `${process.env.NEXT_PUBLIC_APP_URL ?? "http://localhost:3000"}/dashboard/labeling`
    );
  }

  const res = await fetch(`${BACKEND_URL}/api/platform/email-actions/${encodeURIComponent(token)}`, {
    method: "GET",
    redirect: "manual",
  });
