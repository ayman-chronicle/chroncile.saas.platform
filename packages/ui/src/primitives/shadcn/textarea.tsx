...
export const textareaVariants = cva(
  "min-h-[120px] w-full resize-y border outline-none transition-[border-color,box-shadow,background-color] duration-fast ease-out disabled:cursor-not-allowed disabled:opacity-50 data-[invalid=true]:border-event-red data-[focused=true]:data-[invalid=true]:border-event-red data-[disabled=true]:cursor-not-allowed data-[disabled=true]:opacity-50",
  {
    variants: {
      density: {
        compact:
          "rounded-l border-l-border bg-l-surface-input px-[10px] py-[6px] font-sans text-[13px] leading-snug text-l-ink placeholder:text-l-ink-dim hover:border-l-border-strong focus:border-[rgba(216,67,10,0.5)] focus:shadow-[0_0_0_3px_rgba(216,67,10,0.12)] data-[hovered=true]:border-l-border-strong data-[focused=true]:border-[rgba(216,67,10,0.5)] data-[focused=true]:shadow-[0_0_0_3px_rgba(216,67,10,0.12)]",
        brand:
          "rounded-sm border-hairline-strong bg-surface-00 px-s-3 py-s-2 font-mono text-mono-lg text-ink placeholder:text-ink-faint hover:border-ink-dim focus:border-ember data-[hovered=true]:border-ink-dim data-[focused=true]:border-ember",
      },
      variant: {
        default: "",
...
export const ShadcnTextarea = React.forwardRef<
  HTMLTextAreaElement,
  ShadcnTextareaProps
>(({ className, density, invalid, variant, ...props }, ref) => (
  <textarea
    ref={ref}
    className={cn(textareaVariants({ density, invalid, variant }), className)}
